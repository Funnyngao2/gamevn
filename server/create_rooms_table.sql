...
--
-- IMPORTANT: chat_messages for a room are deleted automatically when the room
-- is deleted (ON DELETE CASCADE via room_id FK on game_rooms).
-- Lobby messages (room_id IS NULL) are kept permanently.
...
-- ── Chat messages ─────────────────────────────────────────────────────────────
-- channel='lobby' : global chat, room_id IS NULL, kept forever
-- channel='room'  : per-room chat, room_id = game_rooms.id
--                   deleted automatically when the room row is deleted (CASCADE)
CREATE TABLE IF NOT EXISTS chat_messages (
  ...
  -- Room messages cascade-delete when the room is removed
