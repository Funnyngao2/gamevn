...
import { SceneBg, COLOR_HEX } from './../views/lobbyShared.jsx'
...
export default function PhaserGame({ visible }) {
  ...
  useEffect(() => {
    ...
    const launchGame = (Phaser, scenes) => {
      ...
      const { PreloadScene, GameScene } = scenes
      game = new Phaser.Game({
        ...
        // React handles in-game UI overlays; Phaser only handles gameplay/map
      ...
      game.registry.set('assignedTasks', gameData.assignedTasks || [])
      if (gameData.spawnX != null && gameData.spawnY != null) {
        game.registry.set('spawn', { x: gameData.spawnX, y: gameData.spawnY })
      }
      game.registry.set('socket',      getSocket())
      game.registry.set('onGameEnd',   (result) => endGame({
        winner: result?.winner,
        roomId: result?.roomId || gameData.roomId,
        players: result?.players || [],
        localPlayerId: result?.localPlayerId || getSocket()?.id,
      }))
      ...
      // Khởi tạo placeholders cho cuộc họp để tránh lỗi race condition
      game.registry.set('onMeetingStart', () => {})
      game.registry.set('onMeetingState', () => {})
      game.registry.set('onMeetingVote', () => {})
      game.registry.set('onMeetingChat', () => {})
      game.registry.set('onMeetingResult', () => {})
    ...
    ]).then(([{ default: Phaser }, { PreloadScene }, { GameScene }]) => {
      launchGame(Phaser, { PreloadScene, GameScene })
    })
  }, [visible, gameData])
